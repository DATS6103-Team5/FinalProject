...
model_imputed_df = simple_imputed_df.dropna(subset=['review_scores_rating'])

# Now, calculate the mean review scores rating for each neighborhood
best_neighborhoods = simple_imputed_df.groupby(['City', 'neighbourhood_cleansed'])['review_scores_rating'].mean().reset_index()
...
top_listings = pd.DataFrame(simple_imputed_df)
...
best_hosts = simple_imputed_df.groupby(['City', 'host_id'])['review_scores_rating'].mean().reset_index()
...
# Assuming you have the 'model_imputed_df' DataFrame with the necessary data

# Compute mean price for each neighborhood
neighborhood_impact = model_imputed_df.groupby('neighbourhood_cleansed')['price'].mean().reset_index()
neighborhood_impact = neighborhood_impact.rename(columns={'price': 'mean_price'})

# Get the top 10 neighborhoods based on mean price
top_10_neighborhoods = neighborhood_impact.nlargest(10, 'mean_price')['neighbourhood_cleansed'].tolist()
